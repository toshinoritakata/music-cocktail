...
fetch('/api/status').then(r=>r.json()).then(s=>{mode=s.mode;$('mode').textContent=mode==='jev'?'● Jev / 全6項目方式':'○ デモ / ルールで選択';}).catch(()=>{$('mode').textContent='接続エラー';});
